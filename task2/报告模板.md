...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    {
        if(huffman[k][3] == -1 && huffman[k][0] == -1)  /*补充判断模块注释（说明作用）*/判断此时没有父节点与赋值
        {
            break;
        }
        if(huffman[k][3]<min2 && huffman[k][0]==-1 && k!=ii[0])
        {
            min2=huffman[k][3]
                ii[1]=k;
        }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
       ...
       {code[start]='1'}/*自行填充代码*/		//判断当前结点的父结点左子树是否为当前结点
       else if(huffman[father][2]==current)
       {code[start]='0';/*自行填充代码*/		//子结点是父结点的左子树，编码为0
        /*自行填充代码*/		//子结点是父结点的右子树，编码为0

        father=current;/*自行填充代码*/		//往上朔源，更新当前结点
        huffman[father][0]=huffman[current][0];/*自行填充代码*/		//同理（当前结点更新后），更新当前结点的父亲结点
        start++;/*自行填充代码*/		//更新编码位置
    }
    code[start]='\0';   //编码结束符
...
（1）DEAEDDEDBCEEECECDEEDDBEEECEEDEE   56  A 0000 B 0001 C 001D 01 E1

（2）EEBEACCCCAADACCBCCCCECCCEEECCCE    56  A 001 0001 C 1 D 0000 E 01

---

### 思考题

1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？叶节点为n，生成的父节点为n-1，所以节点总数为2n-1
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？改成0*61+j   将所有大写字母改成小写字母运算或者将小写字母改成大写字母运算
3. 
4. 使用画图工具![huffman1](C:\Users\52978\Desktop\huffman1.png)画出上述两个Huffman生成树的生成过程

   ![huffman2](C:\Users\52978\Desktop\huffman2.png)
